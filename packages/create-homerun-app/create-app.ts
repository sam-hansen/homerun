...
export async function createApp({
    ...
    try {
        ...
        console.error(
            `The application path is not writeable, please check folder permissions and try again`
        );
        process.exit(1);
    }
    ...
    try {
        ...
        console.error(
            `${chalk.red(
                chalk.bold("Error !")
            )} Could not create application path. Exiting`
        );
        process.exit(1);
    }
    process.chdir(root);
    ...
    console.log("\n");
    console.log(
        `    ${chalk.cyan(chalk.bold("npm"))} ${chalk.dim("run")} build`
    );
    console.log(`    Build the app for production`);
    console.log("\n");
    console.log(
        `    ${chalk.cyan(chalk.bold("npm"))} ${chalk.dim("run")} start`
    );
